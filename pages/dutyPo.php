         ...
         <label for="tab2">Consumption Report</label>  -->         
          <section id="content1">
            <form class="" action="../php/duty.php" method="POST">
              <input type="text" name="againstPo" style="display: none;" value="true">
              <div class="col-md-3">
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <input class="form-control" name="discount" value="0.00" type="text"/>                  
                </div>
              </div>
              <div class="col-md-3">
                <div class="form-group">
...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
  <script type="text/javascript">
    ...
    $("body").on("click", "#upload", function () {
        ...
        function ProcessExcel(data) {
            ...
            for (var i = 0; i < excelRows.length; i++) {
                ...
                var v = parseInt((excelRows[i].Total_Amount)/(excelRows[i].Qty)).toFixed(2);
                $("tbody#inwardparts").append("<td><input type='text' class='form-control' name='Landedcostper"+i+"' value='"+v+"' readonly/></td>");
    ...
    $(document).ready(function(){
      ...
      $("table.or").on("keyup", ".quantity", function (event) {        
          if(cur=="inr"){
            var id = $(this).attr("id");
            var unitPrice = $("input[name='unitRateInr"+id+"']").val();            
            $("input[name='landedcost"+id+"']").val(unitPrice*$(this).val());
          }
      });
      ...
      $("table.or").on("change", ".part", function (event) {          
          var id = $(this).attr("id");
          //$("select[name='partno"+id+"']").attr("disabled","disabled");          
          ...
          $.each(JSON.parse(partsData), function(i, part){
            if(part.id==val[0]){
              ...
              /*$.each(JSON.parse(stocks), function(i, stock){
                if(val[1]==stock.part){
                  $("select[name='serial"+id+"[]']").append("<option value='"+stock.id+"'>"+stock.srno+"</option>");
                }
              });*/
            }
          });
     ...
     $("table.or").on("keyup", ".forwarding", function (event) {
        ...
        $("table.or").on("keyup", ".unitRateEuro", function (event) {
        var id = $(this).attr("id");
        var eurorate = parseFloat($("input[name='euroRate']").val());
        var unitRateEuro = parseFloat($("input[name='unitRateEuro"+id+"']").val());
         $("input[name='unitRateInr"+id+"']").val(eurorate*unitRateEuro);
  ...
  function getPODetails(id){
    ...
    $.ajax({
      method:"POST",
      type:"json",
      data:{"poId":id,"getPO":"getPO"},
      url:"../php/po.php",
      success:function(data){
        //console.log(data);
        cur= JSON.parse(data)[0].currency;
        // dataR = JSON.parse(data);
        // console.log(dataR);
        if(cur=="inr"){
          $("input[name='euroRate']").val("1");
        }else{
          $("input[name='euroRate']").val($.cookie("eurorate"));
        }        
        $("input[name='euroRate']").val(JSON.parse(data)[0].eurorate);
        $("select[name='vendor']").val(JSON.parse(data)[0].to);
        $("#poParticulars").html("");
        var parts=JSON.parse(data)[0].poparts;
        var partsDataObj = JSON.parse(partsData);
        $.each(parts, function(i, poparts){
          // console.log('parts data'+partsData);          
          var pa="";
          var paINR="";
          var paEUR = "";
          $.each(JSON.parse(partsData), function(j, p){
            if(p.id==poparts.partId){            
              pa = p.partno;
              /*paEUR=p.unitpriceeuro;
              paINR=p.unitpriceinr;*/
            }
          });
          //var inr = parseFloat
          ...
          +"<input class='form-control' type='text' name='partid"+partIndex+"' value='"+poparts.partId+"'  readonly>"
          +"</td>"
          +"<td>"
          +"<input class='form-control' type='text' name='partno"+partIndex+"' value='"+pa+"' readonly>"
          +"</td>"
          +"<td>"
          +"<input class='form-control' type='text' name='unitRateEuro"+partIndex+"' value='"+poparts.rate+"' readonly>"
          +"</td>"
          +"<td>"
          +"<input class='form-control' type='text' name='unitRateInr"+partIndex+"' value='"+(JSON.parse(data)[0].eurorate*poparts.rate)+"' readonly>"
          +"</td>"
          +"<td>"
          +"<input class='form-control quantity' type='text' id='"+partIndex+"' name='quantity"+partIndex+"' value='"+poparts.qty+"' >"
          +"</td>"
          +"<td>"
          +"<input class='form-control' type='text' name='duty"+partIndex+"' >"
          +"</td>"
          +"<td>"
          +"<input class='form-control' type='text' name='clearing"+partIndex+"'>"
          +"</td>"
          +"<td>"
          +"<input class='form-control' type='text' name='packging"+partIndex+"'>"
          +"</td>"
          +"<td>"
          +"<input class='form-control forwarding' id='"+partIndex+"' type='text' name='forwarding"+partIndex+"'>"
          +"</td>"
          +"<td>"
          +"<input class='form-control' type='text' name='Landedcostper"+partIndex+"'>"
          +"</td>"
          +"<td>"
          +"<input class='form-control' type='text' name='landedcost"+partIndex+"'>"
          +"</td>"
          +"</tr>";
          //console.log(cur+"   "+partIndex);          
          $("#poParticulars").append(element);
          if(cur=="inr"){
            $("input[name='duty"+partIndex+"']").prop("readonly",true);
            $("input[name='clearing"+partIndex+"']").prop("readonly",true);
            $("input[name='packging"+partIndex+"']").prop("readonly",true);
            $("input[name='forwarding"+partIndex+"']").prop("readonly",true);
            ...
            $("input[name='Landedcostper"+partIndex+"']").val(paINR);
            $("input[name='landedcost"+partIndex+"']").val(parseFloat(paINR*poparts.qty));
          }
          addPartIds.push(partIndex);
          partIndex++;
        });              
