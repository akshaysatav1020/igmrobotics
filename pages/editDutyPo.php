  ...
  <?php include "menu.php";?>
    <div class="container">
      ...
      <form class="edit" action="../php/duty.php" method="POST">
        ...
        <input type="text" value="true" name="againstPo" style="display: none;">
        <div class="col-md-3">
                <div class="form-group">
  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
      <form class="editForm" action="../php/duty.php" method="post">
        ...
        <input type="text" name="eAgainstPo" value='true' style="display: none" />
        <input type="text" style="display: none" name="eDutyId" value="<?php echo $_GET['id']; ?>"/>
        <div class="col-md-3" style="display: none">
          <div class="form-group">
            <label for="custid">Duty Id</label>
            <input type="text" class="form-control" name="eDutyId" value="<?php echo $_GET['id']; ?>" readonly />
          </div>
        </div>
        <div class="col-md-3" style="display: none">
          <div class="form-group">
            <label for="custid">PO Id</label>
        ...
        <div class="col-md-3">
          <div class="form-group">
            <label for="custid">Part No.</label>
            <input type="text" class="form-control" name="ePart" readonly />
          </div>
        </div>
        <div class="col-md-3">
          <div class="form-group">
            <label for="companyname">Unit Rate (INR)</label>
        ...
        <div class="col-md-3">
            <div class="form-group">
              <label for="lastname">Duty Amt.</label>
              <input class="form-control" name="eDuty"/>
            </div>
          </div>
          <div class="col-md-3">
            <div class="form-group">
              <label for="lastname">Clearing Amt.</label>
              <input class="form-control" name="eClearing"  />
            </div>
          </div>
        <div class="col-md-3">
          <div class="form-group">
            <label for="lastname">Packaging Amt.</label>
            <input type="text" class="form-control number" name="ePackaging" />
          </div>
        </div>
        <div class="col-md-3">
          <div class="form-group">
            <label for="lastname">Forwarding Amt.</label>
            <input type="text" class="form-control" name="eForwarding"   />
          </div>
        </div>
        <div class="col-md-3">
          <div class="form-group">
...
<div class="loader">
    ...
    $(".loader").show();
      (function(){
        $(window).bind("load", function() {
          checkNetworkStatus();
           $("select[name='vendor'],select[name='po']").select2({"width":"100%"});
        });
        ...
        $(document).ready(function(){
          ...
          $("input[name='forwarding']").on("keyup",function(){            
          ...
          $("input[name='eForwarding']").on("keyup",function(){                        
    ...
    function getDuty(){
        ...
        $.ajax({
            ...
            $("input[name='id']").val(info.dutyDetails.duty_id);
            $("select[name='po']").val([info.dutyDetails.po]).trigger("change");
            $("input[name='eDutyPO']").val(info.dutyDetails.po);
            $("input[name='billOfEntryNo']").val(info.dutyDetails.bill_of_entry_no);
            $("input[name='billOfEntryDate']").val(info.dutyDetails.bill_of_entry_date.split(" ")[0]);
            $("input[name='invoiceNo']").val(info.dutyDetails.invoice_no);
            $("select[name='vendor']").val([info.dutyDetails.vendor]).trigger("change");
            $("input[name='euroRate']").val(info.dutyDetails.euro_rate);
            $("input[name='inwardDate']").val(info.dutyDetails.inward_date.split(" ")[0]);
            $("input[name='inwardNo']").val(info.dutyDetails.inward_no);
            $("input[name='discount']").val(info.dutyDetails.discount);
            inwardnoParticular=info.dutyDetails.inward_no;
            
            $.each(info.partDetails, function(i, dutyParts){
              var element = "";
              element = "<tr>"              
              +"<td>"
              +"<input class='form-control' style='width:150px' type='text' value='"+dutyParts.part_no+"' readonly='readonly'/>"
              +"</td>"
              +"<td>"
              +"<input class='form-control' type='text' id='"+dutyParts.id+"' value='"+dutyParts.unit_rate_euro+"' readonly>"
              +"</td>"
              +"<td>"
              +"<input class='form-control' type='text' id='"+dutyParts.id+"' value='"+dutyParts.unit_rate_inr+"' readonly>"
              +"</td>"
              +"<td>"
              +"<input class='form-control' type='text' value='"+dutyParts.qty+"' readonly/>"
              +"</td>"
              +"<td>"
              +"<input class='form-control' style='width:100px' type='text' value='"+dutyParts.duty+"' readonly/>"
              +"</td>"
              +"<td>"
              +"<input class='form-control' type='text' value='"+dutyParts.clearing_charges+"' readonly/>"
              +"</td>"
              +"<td>"
              +"<input class='form-control' type='text' value='"+dutyParts.packaging+"' readonly/>"
              +"</td>"
              +"<td>"
              +"<input class='form-control' type='text' value='"+dutyParts.forwarding_charges+"' readonly/>"
              +"</td>"
              +"<td>"
              +"<input class='form-control' type='text' value='"+dutyParts.landed_cost_per_part+"' readonly/>"
              +"</td>"
              +"<td>"
              +"<input class='form-control' type='text' value='"+dutyParts.total_landed_cost+"' readonly/>"
              +"</td>"
              +"<td>"
              +"<a onclick='updatePart("+dutyParts.duty_particluar_id+")'>Edit</a>&emsp;"
              +"<a onclick='deletePart("+dutyParts.duty_particluar_id+")'>Remove</a>"
              +"</td>"
              +"</tr>";
