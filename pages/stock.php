...
<html>
  ...
  <?php include "menu.php";?>
    <div class="container-fluid" >
      <div class="row">
        <main>
           <?php if($_COOKIE["userrole"]=="finance"||$_COOKIE["userrole"]=="engineer"||$_COOKIE["userrole"]=="service"){ ?>
            <input id="tab6" type="radio" name="tabs" checked>
          <label for="tab6">View Stock</label>

          <?php }else if($_COOKIE["userrole"]!="service"|| $_COOKIE["userrole"]!="engineer" || $_COOKIE["userrole"]!="viewer"){ ?>
               <input id="tab5" type="radio" name="tabs" checked>
          <label for="tab5">Add Stock</label>
          <input id="tab6" type="radio" name="tabs">
          <label for="tab6">View Stock</label>
          <input id="tab2" type="radio" name="tabs">
          <label for="tab2">View Serials</label>
          <input id="tab3" type="radio" name="tabs">
          <label for="tab3">Import Stock</label>                  
          <?php } ?>
          ...
          <section id="content6">
            <table class="table table-bordered table-striped table-condensed" id="myTable1">
                ...
                <tbody>
                  <!-- <?php                                            
                      /*$query = "SELECT COUNT(id) as count, part_number, part_id FROM inventory group by part_number";
                      $result = $connection->query($query);
                      $data = array();                
                      if($result->num_rows>0){
                        while($row=$result->fetch_assoc()){
                          ...
                          //echo "<td>".getPartDetails($connection, $row["part_number"],"total")."</td>";
                          echo "<td>".getPartDetails($connection, $row["part_id"],"min")."</td>";
                          echo "<td>".getPartDetails($connection, $row["part_id"],"location")."</td>";
                          echo "<td>".getPartDetails($connection, $row["part_id"],"country")."</td>";                          
                          echo "</tr>";
                        }
                      }                              
                      

                function getPartDetails($connection, $part, $col){
                        $pono="";
                        if($col=="min"){
                          $query = "SELECT COUNT($col) as min FROM inventory_parts WHERE id=$part";
                        }else if($col=="description"){
                          $query = "SELECT $col FROM inventory_parts WHERE id=$part";
                        }else if($col=="location"||$col=="country"){
                          $query = "SELECT $col as $col FROM inventory_parts WHERE id=$part";
                        }
                        $result = $connection->query($query);
                        $data = array();                
                        if($result->num_rows>0){
                  ...
                  <?php                      
                     /* $query = "SELECT * FROM inventory";
                      ...
                      function getPO($connection, $id){
                        $pono="";
                        $query = "SELECT po_no FROM purchaseorder WHERE id=$id";
                        $result = $connection->query($query);
                        $data = array();                
                        if($result->num_rows>0){
                          while($row=$result->fetch_assoc()){                            
      ...
      <div class="modal-body">
        <form method="POST" action="../php/warehouseStock.php">
              ...
              <div class="col-md-3">
                  <div class="form-group">
                    <label for="description">Serial No</label>
                    <input type="text" class="form-control" name="eserno" />
                  </div>
                </div>                
                <div class="col-md-3">
                  <div class="form-group">
                    ...
                    <select class="form-control" name="epartno">
                      <option>Select Part</option>
                      <?php                      
                       /* $query = "SELECT id,part_number FROM inventory_parts";
                        $result = $connection->query($query);
                        $data = array();                
                        if($result->num_rows>0){
                          while($row=$result->fetch_assoc()){                            
                ...
                <div class="col-md-3">
                  <div class="form-group">                    
                    ...
                    <select class="form-control" name="einwardno">
                      <option value="">Select</option>
                      <?php                      
                        $query = "SELECT inward_no FROM duty";
                        $result = $connection->query($query);
                        $data = array();                
                        if($result->num_rows>0){
                          while($row=$result->fetch_assoc()){                            
                ...
                <div class="col-md-3">
                  <div class="form-group">
                  <label for="description">Purchase Order No</label>
                  <select class="form-control"  name="epono" >
                    <option>Select Purchase Order</option>
                    <?php                      
                        /*$query = "SELECT id,po_no FROM purchaseorder";
                        $result = $connection->query($query);
                        $data = array();                
                        if($result->num_rows>0){
                          while($row=$result->fetch_assoc()){                            
                ...
                <div class="col-md-3">
                  ...
                  <input type="text" class="form-control" name="echno"/>
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="form-group">
                  <label for="description">Challan Date</label>                  
                  <input type="text" class="form-control" name="echdate" />
                  </div>
                </div>
                ...
                <div class="col-md-4">
                  ...
                  <input type="text" class="form-control" name="eImportDate" />
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
      ...
      jQuery.fn.DataTable.Api.register( 'buttons.exportData()', function ( options ) {
        ...
        var rs = '';
            if ( this.context.length ) {
              if(this.context[0].sTableId == 'myTable1'){
                var jsonResult = $.ajax({
                    url: '../php/getStockInventoryData.php?page=all',
                    data: {search: {value:$("#myTable1_filter input").val()}},
                    method:'post',
                ...
                return {body: rs.aaData, header: $("#myTable1 thead tr th").map(function() { return this.innerHTML; }).get()};

              } else {
                var jsonResult = $.ajax({
                    url: '../php/getStockPageData.php?page=all',
                    data: {search: {value:$("#myTable2_filter input").val()}},
                    method:'post',
                    success: function (result) {
                        rs= JSON.parse(result);
                ...
                return {body: rs.aaData, header: $("#myTable2 thead tr th").map(function() { return this.innerHTML; }).get()};
      ...
      if($.cookie("userId")!="15"){
        ...
        $('#myTable1').DataTable({
          dom: 'Bfrtip',
            ...
            'serverMethod': 'post',
            'ajax': {
              'url':'../php/getStockInventoryData.php',
        ...
        dom: 'Bfrtip',
          buttons: [
              ...
              $.extend( true, {}, buttonCommon, {
                  extend: 'excelHtml5',title:"Returnable List",exportOptions:{columns:[0,1,2,3,4]}
              } ),
              $.extend( true, {}, buttonCommon, {
                  extend: 'pdfHtml5',title:"Returnable List",exportOptions:{columns:[0,1,2,3,4]}
              } )
          ],
           'processing': true,
            'serverSide': true,
          ...
          'columns': [
             ...
             render: function ( data, type, row ) {
              //console.log(row);
        return '<button class="btn btn-xs btn-warning edit" onClick="editStockPart('+row.id+')" >Edit/View</button>&emsp;<button class="btn btn-xs btn-danger delete" onClick="deleteStockPart('+row.id+')">Delete</button>';
        ...
        $.ajax({
          ...
          success:function(data){ 
            ...
            $.each(JSON.parse(parts),function(i,part){
              //console.log(part);
              $("select[name='partno'],select[name='epartno']").append("<option value='"+part.id+"'>"+part.partno+"</option>");
            });
            $("select[name='partno'],select[name='epartno']").select2({width:"100%"});
            getPO();
          },error:function(){
      ...
      $.ajax({
        ...
        success:function(data){
          ...
          if(JSON.parse(data).length>0){
            ...
            $("select[name='pono'],select[name='epono']").select2({width:"100%"});
    ...
    function getInward(){
      $.ajax({
        ...
        success:function(data){
          ...
          if(JSON.parse(data).length>0){
            poList = JSON.parse(data);
            $("select[name='inwardno']").html("<option value=''>Select Inward</option>");              
            $.each(JSON.parse(data),function(i,inward){
            $("select[name='inwardno'],select[name='einwardno']").append("<option value='"+inward.duty_id+"'>"+inward.inward_no+"</option>");                            
            });
            $("select[name='inwardno'],select[name='einwardno']").select2({width:"100%"});
