 ...
 <link href="../css/select2.min.css" rel="stylesheet" />
  ...
  <?php include "menu.php";?>
    <div class="container-fluid" >
      <div class="row">
        <main>
          ...
          <label for="tab1">Generate Credit Note</label>
            ...
            <form class="" action="../php/cn.php" method="post">
              ...
              <div class="col-md-4">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="cnno"   />
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
                  <label for="description">Invoice Number</label>
                  <input type="text" class="form-control" name="refno"  placeholder="Enter Purchase Order No." />
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
          ...
          <section id="content2">
            <div class="table-responsive">
              <table class="table table-bordered table-striped table-condensed" id="myTable">
                ...
                <tbody>
                  <?php
                      $query="SELECT cn.*, cust.company_name as cname, (SUM(cnp.partTotAmount)+((cgst/100)*SUM(cnp.partTotAmount))+((sgst/100)*SUM(cnp.partTotAmount))+((igst/100)*SUM(cnp.partTotAmount))) as totalAmt FROM `creditnote` cn LEFT JOIN customers cust on cn.cust=cust.id LEFT JOIN cn_parts cnp on cn.id=cnp.cnId GROUP BY cn.id";
                      $result = $connection->query($query);
                      $data = array();
                      if($result->num_rows>0){
                        while($row=$result->fetch_assoc()){                    
                          ...
                          if($_COOKIE["status"]=="Online"){
                            echo "<button class='btn btn-sm btn-success' onclick='printCN(".$row["id"].")'>Print</button>&emsp;";
                            echo "<button class='btn btn-sm btn-warning' onclick='editCN(".$row["id"].")'>Edit</button>&emsp;";
                            echo "<button class='btn btn-sm btn-danger' onclick='deleteCN(".$row["id"].")'>Delete</button></td>";
                          }else{                        
                            echo "<button class='btn btn-sm btn-success' onclick='printCN(".$row["id"].")'>Print</button>&emsp;";
                            echo "<button class='btn btn-sm btn-warning' onclick='editCN(".$row["id"].")' disabled>Edit</button>&emsp;";
                            echo "<button class='btn btn-sm btn-danger' onclick='deleteCN(".$row["id"].")'disabled>Delete</button></td>";
                          }
                          echo "</tr>";
                        }
                      } 
                      ...
                      function getTotalAmount($connection, $id){
                        $query="SELECT SUM(partTotAmount) as total FROM cn_parts WHERE cnId=".$id;
                        $result = $connection->query($query);
                        $data = "";
                        if($result->num_rows>0){
                          while($row=$result->fetch_assoc()){          
    ...
    <script type="text/javascript">
      //add edit delete cnId get getAll
    ...
    $(document).ready(function(){
      ...
      $("table.or").on("input", ".rate", function (event) {
          var qty = $("input[name='qty"+$(this).attr("id")+"']").val("0");
          var unitPrice = $("input[name='rate"+$(this).attr("id")+"']").val();
          //console.log(qty+"    "+unitPrice);
...
function addPart(){
      ...
      +"</td>"
      +"<td>"
      +"<input type='text' class='form-control rate number' id='"+partIndex+"' name='rate"+partIndex+"' required/>"
      +"</td>"
      +"<td>"
      +"<input type='text' class='form-control qty number' name='qty"+partIndex+"' id='"+partIndex+"' required/>"
      +"</td>"
      +"<td>"
            ...
            $('#myTable').DataTable({
              dom: 'Bfrtip',
                buttons: [
                    ...
                    $.extend( true, {}, buttonCommon, {
                        extend: 'excelHtml5',title:"Credit Note List",exportOptions:{columns:[0,1,2]}
                    } ),
                    $.extend( true, {}, buttonCommon, {
                        extend: 'pdfHtml5',title:"Credit Note List",exportOptions:{columns:[0,1,2]}
                    } )
                ]
            });
    ...
    function printCN(id){
      window.open("../php/cn.php?id="+id+"","_blank");
    }

    function editCN(id){
      window.open("editCn.php?id="+id+"","_self");
    }
    ...
    function getPreCNNo(){
        $.ajax({
          method:"POST",
          type:"json",
          data:{"getLastCN":"getLastCN"},
          url:"../php/cn.php",
          success:function(data){            
            if(data==""){
              $("input[name='cnno']").val("CN001");
            }else{        
              if(data!=""){
...
/*$('#selectpicker').selectpicker(alert('dds'));*/
</script>
  </body>
</html>
